...
load("@rules_java//test/java/testutil:artifact_closure.bzl", "artifact_closure")
load("@rules_java//test/java/testutil:java_info_subject.bzl", "java_info_subject")
...
def _test_lite_binary_deps_impl(env, target):
    basenames = [f.basename for f in artifact_closure.of_target(target)]
    env.expect.that_collection(basenames).contains_at_least([
        "foo-lite-src.jar",
        "baz-lite-src.jar",
        "libfoo-lite.jar",
        "libbaz-lite.jar",
...
def _test_lite_java_proto2_compiler_args_impl(env, target):
    ...
    action.argv().contains_at_least([
        "--java_out=lite,immutable:{bindir}/{package}/{test_name}/protolib-lite-src.jar",
        "-I.",
        "{package}/file.proto",
    ]).in_order()
...
def _test_lite_same_version_compiler_arguments_impl(env, target):
    ...
    action.argv().contains_at_least([
        "--java_out=lite,immutable:{bindir}/{package}/{test_name}/bravo-lite-src.jar",
        "-I.",
        "{package}/bravo.proto",
    ]).in_order()

    java_info_subject.from_target(env, target).compilation_args().transitive_runtime_jars().contains_at_least_predicates([
        matching.file_basename_equals("libbravo-lite.jar"),
        matching.file_basename_equals("libprotobuf_lite.jar"),
    ])
...
def _test_lite_jpl_correctly_defines_direct_jars_strict_deps_enabled_alias_proto_impl(env, target):
    java_info_subject.from_target(env, target).compilation_args().compile_jars().contains_exactly([
        "{package}/{test_name}/libbar_proto-lite-hjar.jar",
    ])
