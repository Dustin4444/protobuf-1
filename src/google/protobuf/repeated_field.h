...
class alignas(8) HeapRep {
 public:
  ...
  static constexpr size_t SizeOf() {
    constexpr size_t kMinHeapRepSize = std::max<size_t>(sizeof(Element), 8);
    ...
    return std::max(sizeof(HeapRep), kMinHeapRepSize);
  }
...
// The base class for `RepeatedField`. Used to share common data which can be
// safely cast to any specialization of `RepeatedField`.
