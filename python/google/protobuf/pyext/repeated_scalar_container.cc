...
template <typename T>
static PyObject* AddRange(const Reflection* reflection,
                          const FieldDescriptor* field_descriptor,
                          Message* message, absl::Span<const T> values) {
    ...
    switch (field_descriptor->cpp_type()) {
      case FieldDescriptor::CPPTYPE_INT32:
        if (fmt == 'i' || (fmt == 'l' && view.itemsize == 4)) {
          PyObject* result = AddRange(
              reflection, field_descriptor, message,
